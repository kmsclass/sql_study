/*
  1. 집합연산자
      union    : 합집합. 중복되지 않는다
      inion all: 중복됨. 두개의 쿼리의 결과를 연결하여 조회
   ...
   3. 문자열 관련 함수
	   - 대소문자 변경 : upper,lower
		...
		- 부문문자열    : substr(문자열,시작인덱스,[갯수]),left(문자열,갯수),right(문자열,갯수)
		- 문자연결함수  : concat
...
union
SELECT NAME,POSITION,salary,bonus,salary FROM professor
WHERE bonus IS NULL
-- ifnull 사용 : ifnull(컬럼명,기본값)
-- ifnull(bonus,0) : bonus컬럼의 값이 null인경우 0으로 치환
SELECT NAME,POSITION,salary,bonus,salary+ifnull(bonus,0) FROM professor
-- ifnull(salary+bonus,salary) : salary+bonus의 결과가 null인경우 salary로 치환
SELECT NAME,POSITION,salary,bonus,ifnull(salary+bonus,salary) FROM professor

-- 교수의 이름,직책,급여,보너스 출력하기
-- 보너스가 없는 경우는 보너스없음으로 출력하기
SELECT NAME,POSITION,salary,ifnull(bonus,'보너스없음') bonus FROM professor
...
SELECT NAME,deptno,if(deptno=101,"컴퓨터공학","그외학과") FROM professor
-- 학생의 주민번호 7번째 자리가 1,3인경우 남자, 2,4인경우 여자로 출력하기
SELECT NAME,jumin, 
    if(SUBSTR(jumin,7,1) =1,"남자",
	 if(SUBSTR(jumin,7,1) =2,"여자",
	 if(SUBSTR(jumin,7,1) =3,"남자",
	 if(SUBSTR(jumin,7,1) =4,"여자","주민번호오류")))) 성별
FROM student
...
-- 교수이름,학과코드,학과명 출력하기
-- 학과명 : 101 학과인 경우 '컴퓨터공학', 그외는 그외학과  출력
SELECT NAME,deptno,
     case deptno when 101 then "컴퓨터공학"
	              ELSE "그외학과" END 학과명
FROM professor					   
...
-- if 조건문
SELECT NAME,deptno, if(deptno IN (101,102,201),"공과대학","그외대학")
FROM professor
...
-- 학생의 전체 인원와 지도교수를 배정받은 학생의 인원수를 조회하기
SELECT COUNT(*),COUNT(profno) FROM student 

-- 학생 중 전공1학과가 101인 학과에 속한 학생의 인원수 조회하기
...
-- avg(ifnull(bonus,0)) : bonus값이 null인 경우 0을 환산하여 평균 처리
...
SELECT deptno,SUM(salary),SUM(bonus),SUM(salary*12+ifnull(bonus,0)),
              round(avg(salary),2),
...
/*
  최소값,최대값 :   min,max 
*/
-- 전공1학과별 가장 키가 큰학생의 큰키값관,작은키값 출력하기
...
-- 교수테이블에서 학과별 평균급여가 350이상인 부서의 코드와평균급여 출력하기
SELECT deptno, AVG(salary) FROM professor
GROUP BY deptno
HAVING AVG(salary) >= 350 

-- 주민번호 기준으로 남,여학생의 최대키, 최소키,평균키를 조회하기
...
-- score 테이블에서 학번,국어,수학,영어,총점, 총점기준등수 출력하기
SELECT *,kor+math+eng 총점, RANK() OVER(ORDER BY kor+math+eng DESC) 등수 
FROM score

-- score 테이블에서 학번,국어,수학,영어,총점, 총점기준등수, 과목별 등수 출력하기
...
select grade, avg(weight) 몸무게평균,
      avg(height) 키평균 from student
group by grade

-- 학년별, 성별 몸무게 평균, 키평균 조회하기.  학년별로도 평균조회하기
